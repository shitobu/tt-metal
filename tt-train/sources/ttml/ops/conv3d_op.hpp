...
// Per-dimension (D, H, W) convolution parameter.
...
struct Conv3dPreparedWeight {
    ttnn::Shape weight_shape;  // shape of the [C_out, C_in / groups, kD, kH, kW] weight these were built from
    uint32_t groups = 1;
    uint32_t c_in_block = 0;               // K-row block both forms are laid out with
    std::vector<ttnn::Tensor> forward;     // per group: [kD*kH*kW*C_in_padded, C_out_padded] TILE
    std::vector<ttnn::Tensor> transposed;  // per group, for the input gradient; empty if not prepared
};
