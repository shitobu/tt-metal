...
// Every conv3d activation's channel count must be divisible by the kernel's C_in block, default_c_in_block(kvol),
// which is 16 or 32; a multiple of TILE_WIDTH always satisfies it and matches the alignment
// ttnn::prepare_conv3d_weights pads weights to. C_out is padded with the same value because dY is the activation
// of the transposed convolution in conv3d_input_grad; the forward kernel itself has no C_out requirement and pads
// only so forward and transposed weights are prepared uniformly. Outputs and gradients are sliced back to the
...
// Geometry of one group's dense convolution: same spatial parameters, C_in and C_out are the per-group counts.
// A distinct type so a helper that needs per-group channel counts cannot be handed the whole-op geometry.
...
// Runs `make_group(g)` for every group and collects the results, the one loop shape shared by the forward pass and
// both weight-side gradients.
...
// A weight in the kernel's [kD*kH*kW*C_in_padded, C_out] TILE form together with the K-row block it was laid out
// with. The kernel must be launched with the same C_in_block: a mismatch is not caught by validation and produces
// garbage (ttnn #47316), so the two travel together.
struct PreparedWeight {
    ttnn::Tensor tensor;
    uint32_t c_in_block = 0;
};

// Lays out a dense [C_out_conv, C_in_conv, kD, kH, kW] row-major device weight with ttnn's own preparation so the
// blocking contract has a single implementation. ttnn pads C_in_conv to kChannelAlignment; C_out_conv must already
// be what the kernel is launched with. Stays on device: to_device is a no-op for device tensors and groups == 1
// skips the host-only grouped expansion.
...
// Zero-pads the output-channel axis (dim 0) of a [C_out, C_in, kD, kH, kW] row-major weight. The pad kernel cannot
// address a leading axis of a rank-5 tensor, so this goes through a rank-4 view.
...
// Slices `count` entries starting at `begin` along dimension `dim` of a row-major tensor; no-op for the full range.
...
// Channel slice of a channels-last [N, D, H, W, C] tensor.
...
// Output-channel rows of a [C_out, C_in/groups, kD, kH, kW] weight: group g's own dense weight.
...
// Dense [C_out, C_in, kD, kH, kW] -> prepared forward weight [kD*kH*kW*c_in_padded, c_out_padded] (TILE).
// Output channels are padded to c_out_padded to mirror the transposed weight, whose input side they become in the
// backward pass (see kChannelAlignment).
...
// Builds the weight of the transposed convolution used for dX: input channels become C_out (zero padded to
// c_out_padded), output channels become C_in, and the kernel is flipped along all three spatial axes.
// Result: prepared [kD*kH*kW*c_out_padded, c_in_padded] (TILE).
PreparedWeight prepare_transposed_weight(const ttnn::Tensor& raw_weight, uint32_t c_in_padded, uint32_t c_out_padded) {
    ...
    // Flipping all spatial axes reverses the flattened (kd, kh, kw) index: row k -> row kvol-1-k. ttnn has no flip
    // op and slice rejects negative steps, so this costs kvol single-row slices plus one concat per preparation;
    // callers with a fixed weight avoid repeating it via prepare_conv3d_weight.
    if (kernel_volume > 1U) {
    ...
    // Back to the [out, in, kD, kH, kW] layout ttnn's preparation expects; it permutes again internally, but that
    // keeps the blocking in one implementation.
    weight = ttnn::permute(weight, ttsl::SmallVector<int64_t>{4, 3, 0, 1, 2});
    // ttnn pads only the input side; the output side (C_in) must match the kernel's output_channels, c_in_padded.
    return prepare_weight(pad_output_channels(weight, c_in_padded));
}

// Zero-pads spatial dimension `dim` (1..3) of a channels-last row-major [N, D, H, W, C] tensor by `amount` on both
// sides. Goes through a 4D view so the pad kernel addresses the padded axis directly.
...
// Inserts (stride - 1) zeros between consecutive entries along spatial dimension `dim` (1..3) of a channels-last
// row-major [N, D, H, W, C] tensor and then trims or zero-pads the result to exactly `target_length`.
...
ttnn::Tensor run_ttnn_conv3d(
    ...
    // Same values ttnn::experimental::conv3d would pick on its own, except that C_in_block is pinned to the block
    // the weight was prepared with instead of being re-derived independently.
...
// dX via the transposed convolution: zero-upsample dY by the stride, pad it to the size the transposed kernel
// needs, then run a stride-1 conv3d with the flipped/transposed weight.
ttnn::Tensor conv3d_input_grad(
    ...
    // With the upsampled dY of length L = in + 2p - span (span = dil*(k-1)) and a stride-1 conv with symmetric
    // padding q, output index j holds dX[j - (q - span + p)]. For p <= span choose q = span - p so the output is
    // exactly dX. For p > span the padding q = span is used, the output has length in + 2p, and dX is its
    // crop starting at offset p. Any q in [0, span] would work with crop offset p - span + q; q = span is chosen
    // for symmetry with the other branch.
...
// dW[co, ci, kd, kh, kw] = sum over (n, positions) of dY[n, pos, co] * Xpad[n, pos * stride + k * dilation, ci]
// for one group: dY^T holds the group's C_out output channels, and the group's C_in input channels start at
// `input_channel_begin` of the spatially zero-padded input. One matmul per kernel offset against a strided slice of
// the padded input; nothing leaves the device. `geometry` is the per-group geometry.
...
Conv3dGeometry validate_and_build_geometry(
    ...
    if (weight_shape[1] * groups != geometry.C_in) {
        throw std::invalid_argument(fmt::format(
            "conv3d: weight in_channels {} * groups {} must equal input channels {}",
            weight_shape[1],
            groups,
            geometry.C_in));
    }
    ...
    // The kernel assigns one core per C_in block of its activation. The forward's activation is the (padded,
    // per-group) input; the input gradient's activation is dY, so its channel count is the padded per-group C_out
    // and a wide C_out only fails when dX is required. Report it here rather than as a TT_FATAL inside backward().
...
// Per-group kernel-layout weights for one call. Built from a caller's Conv3dPreparedWeight when given, otherwise
// from the parameter itself; the transposed form is built lazily because only the input gradient needs it.
struct GroupWeights {
...
void validate_prepared_weight(const Conv3dPreparedWeight& prepared, const ttnn::Tensor& weight_value, uint32_t groups) {
...
autograd::TensorPtr conv3d_impl(
    ...
    // Grouped convolution runs as `groups` independent dense convolutions on channel slices. The ttnn kernel has no
    // grouped mode (its own weight preparation expands to a dense block-diagonal weight), so per-group execution is
    // what keeps weight memory at the size of the grouped weight instead of groups times that.
    const auto& input_value = input->get_value();
    const GroupGeometry group = geometry.per_group();
    const uint32_t c_in_padded = group.c_in_padded();
    const uint32_t c_out_padded = group.c_out_padded();
    ...
    auto group_outputs = map_groups(geometry.groups, [&](uint32_t g) {
        // Unaligned input channels cost one zero-padding copy of the (group's) activation per forward.
    ...
    // The forward forms are not needed after this point; only the (possibly absent) transposed forms are kept alive
    ...
    autograd::GradFunction grad = [input, weight, bias, out, geometry, transposed_weights]() {
        ...
        if (needs_weight_grad || needs_bias_grad) {
            ...
            // Full [M, C_out] TILE dY: needed by the bias reduction, and it is also the single group's dY when
            // groups == 1, so it is built at most once.
            ...
            if (needs_weight_grad) {
                ...
                auto group_grads = map_groups(geometry.groups, [&](uint32_t g) {
                    ...
                    // One explicit transpose per group. ttnn::matmul's transpose_a is fused only for the reuse
                    // program configs; otherwise it materializes a transpose per call, which inside the per-offset
                    // loop of conv3d_weight_grad would be kernel_volume transposes instead of one.
...
Conv3dPreparedWeight prepare_conv3d_weight(const ttnn::Tensor& weight, uint32_t groups, bool with_transposed) {
    ...
    // Only the channel and kernel fields matter to weight preparation.
    Conv3dGeometry geometry;
...
autograd::TensorPtr conv3d(
    ...
    const Conv3dGeometry geometry =
        validate_and_build_geometry(input, weight, bias, stride, padding, dilation, groups, padding_mode);
    validate_prepared_weight(prepared, weight->get_value(), groups);
    GroupWeights weights;
