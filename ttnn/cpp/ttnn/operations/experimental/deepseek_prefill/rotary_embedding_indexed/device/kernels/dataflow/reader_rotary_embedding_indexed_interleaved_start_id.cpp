...
// (chunk_local_t may be larger than the input Ht when queries are TP-subsharded), so each device's shard already holds,
// in contiguous local-row order, the rope values for every global position that device will ever carry. This kernel
// only needs to derive WHERE in that shard the current chunk starts -- `update_idxt` -- exactly as the per-chip
